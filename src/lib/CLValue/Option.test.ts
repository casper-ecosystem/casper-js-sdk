...
import { CLValueParsers, CLBool, CLOption, CLBoolType, CLOptionType } from './index';
...
describe('CLOption', () => {
  ...
  it('toBytes() / fromBytes()', () => {
    ...
    expect(CLValueParsers.toBytes(mySomeOpt).unwrap()).to.be.deep.eq(Uint8Array.from([1, 1]));
    expect(optionFromBytes.unwrap()).to.be.deep.eq(mySomeOpt);
    expect(CLValueParsers.toBytes(myNoneOpt).unwrap()).to.be.deep.eq(Uint8Array.from([0]));
  });
  ...
  it('fromJSON() / toJSON()', () => {
    ...
    expect(CLValueParsers.fromJSON(expectedJsonSome).unwrap()).to.be.deep.eq(mySomeOpt);
    expect(CLValueParsers.fromJSON(expectedJsonNone).unwrap()).to.be.deep.eq(myNoneOpt);
  });
});
